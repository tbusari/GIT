...
class CheckCMDStatus < Sensu::Plugin::Check::CLI
  ...
  option :warn,
    :description => 'Fire a warning instead of a critical event',
    :short => '-w',
    :long => '--warn'
  ...
  def get_cmd_status
    ...
    unless $?.exitstatus.to_s == config[:status]
      if config[:warn]
          warning "#{config[:command]} exited with #{$?.exitstatus}"
      else
      ...
      if config[:check_output]
        if Regexp.new(config[:check_output]).match(stdout)
          ...
          if config[:warn]
              warning "#{config[:command]} output didn't match #{config[:check_output]} (exit #{$?.exitstatus})"
          else
